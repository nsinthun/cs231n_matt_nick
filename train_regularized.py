...
def encoder(X):
    ...
    with tf.variable_scope("encoder"):
        ...
        ###########
        c = num_filters
        out = tf.layers.conv2d(X, c, [4,4], strides=[2,2], padding='SAME',kernel_regularizer=regularizer, use_bias=False,name="Wconv1") # + bconv1
        ...
        out = tf.layers.conv2d(out, int(2*c), [4,4], strides=[2,2], padding='SAME',kernel_regularizer=regularizer, use_bias=False,name="Wconv2") # + bconv2 #(Don't Need bias since using batch norm)
        ...
        #Convolution
        c = int(out.get_shape()[-1])
        out = tf.layers.conv2d(out, int(2*c), [4,4], strides=[2,2], padding='SAME',kernel_regularizer=regularizer, use_bias=False,name="Wconv3")
        ...
        #Convolution
        c = int(out.get_shape()[-1])
        out = tf.layers.conv2d(out, int(2*c), [4,4], strides=[2,2], padding='SAME',kernel_regularizer=regularizer, use_bias=False,name="Wconv4")
        ...
        #Convolution
        c = int(out.get_shape()[-1])
        out = tf.layers.conv2d(out, int(c), [4,4], strides=[2,2], padding='SAME',kernel_regularizer=regularizer, use_bias=False,name="Wconv5") # + bconv3 #(Don't Need bias since using batch norm)
...
def run_model(sess, y_out, mean_loss, X_train, y_train, epochs=1, batch_size=64, print_every=100,
    ...
    for e in range(epochs):
        ...
        #Save the model
        saver.save(sess, "model_v4_1/model.ckpt")
